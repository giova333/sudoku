...
namespace Sudoku.Game.Screens
{
    ...
    public sealed class HomeView : MonoBehaviour, IScreen
    {
        ...
        public static HomeView Create(Transform parent)
        {
            ...
            view._continue = AddButton(rect, CopyTable.HomeContinue, ThemeSlot.PrimaryFill, ThemeSlot.PrimaryText);
            ...
            view._newGame = AddButton(rect, CopyTable.HomeNewGame, ThemeSlot.ButtonFill, ThemeSlot.ButtonText);
            ...
            view._daily = AddButton(rect, CopyTable.HomeDaily, ThemeSlot.ButtonFill, ThemeSlot.Disabled);
            view._daily.interactable = false;

            view._settings = AddButton(rect, CopyTable.HomeSettings, ThemeSlot.ButtonFill, ThemeSlot.ButtonText);
        ...
        void Layout()
        {
            ...
            {
                Ui.StackAt(_continue, y);
                Ui.Place(_continueDetail.rectTransform, new Vector2(0, y - 74f), new Vector2(640, 34));
                y -= RowStep;
            }

            Ui.StackAt(_newGame, y);
            Ui.StackAt(_daily, y - RowStep);
            Ui.StackAt(_settings, y - RowStep * 2f);
        }

        /// <summary>A row of the menu, built where it lands later - the rows are
        /// laid out at run time, because how many there are depends on whether
        /// there is a puzzle to continue.</summary>
        static Button AddButton(Transform parent, string text, ThemeSlot fill, ThemeSlot textSlot) =>
            Ui.ScreenButton(parent, text, 0, fill, textSlot);
    }
}
