...
namespace Sudoku.Game.Screens
{
    ...
    public sealed class SettingsView : MonoBehaviour, IScreen
    {
        ...
        public static SettingsView Create(Transform parent, GameSettings settings)
        {
            ...
            // row rather than at a fixed height, so a row added to the list
            // does not land on top of it.
            var note = Ui.Label("Note", rect, 22, ThemeSlot.Muted);
            Ui.Place(note.rectTransform, new Vector2(0, -530), new Vector2(900, 40));
            note.text = CopyTable.SettingsNote;

            var back = Ui.Button("Back", rect, CopyTable.SettingsBack, 28,
            ...
            Action<Action<bool>> observe, Action flip)
        {
            ...
            var toggle = Ui.Button("Toggle", row, string.Empty, 28,
                ThemeSlot.ToggleOff, ThemeSlot.ButtonText);
